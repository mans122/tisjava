	...
	public void actionPerformed(ActionEvent e) {
		...
		case "등록":
			for(int i=0;i<3;i++) {//tf_num값 비어있는게 있는지 확인
				if(Haksa2.tf_num[i].getText().length()==0 || Haksa2.tf_num[i].getText().equals("")) {
		...
		case "목록":
			try {
				...
				while(rs.next()) {
					...
					Haksa2.taList.append(rs.getString("id")); //칼럼 이름대신 칼럼 인덱스도 사용 가능
					Haksa2.taList.append("	");
					Haksa2.taList.append(rs.getString("name"));
					Haksa2.taList.append("	");
					Haksa2.taList.append(rs.getString("dept"));
					Haksa2.taList.append("	");
					Haksa2.taList.append(rs.getString("address"));
					Haksa2.taList.append("\n");
				}
				rs.close();
				conn.close();
