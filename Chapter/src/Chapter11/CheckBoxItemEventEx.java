...
public class CheckBoxItemEventEx extends JFrame {
	...
	public CheckBoxItemEventEx() {
		...
		c.add(new JLabel("사과100원, 배500원, 체리 20000원"));

		//3개의 체크박스를 생성.컨텐트팬에 삽입하고 Item리스너 등록
		...
		for(int i=0;i<fruits.length;i++) {
			...
			fruits[i].addItemListener(listener); //체크박스에 Item리스너 등록
		}

		sumLabel = new JLabel("현재 0원입니다."); //가격합을 출력하는 레이블 생성
		c.add(sumLabel);
	...
	class MyItemListener implements ItemListener{
		...
		public void itemStateChanged(ItemEvent e) {
			if(e.getStateChange() == ItemEvent.SELECTED) {
				...
				if(e.getItem()==fruits[0]) {sum-=100;}
				else if(e.getItem()==fruits[1]) {sum-=100;}
				else {sum-=20000;}
			}
